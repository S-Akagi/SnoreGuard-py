...
class StageRecorder:
    ...
    def set_callbacks(self, progress_callback=None, volume_callback=None, completion_callback=None):
        """UIコールバックを設定"""
    ...
    def record_stage_async(self, stage_name: str, duration: float) -> bool:
        ...
        try:
            ...
            audio_data = np.concatenate(self.audio_chunk_buffer, axis=0).flatten().astype(np.float32)
            
            # 品質チェック
            rms_level = np.sqrt(np.mean(audio_data**2))
            max_amplitude = np.max(np.abs(audio_data))
            
            logger.info(f"録音品質 - RMS: {rms_level:.4f}, Max振幅: {max_amplitude:.3f}")
...
class FeatureAnalyzer:
    ...
    def __init__(self, sample_rate: int = 16000):
        ...
        self.hop_length = 240
        self.sos_filter = butter(N=5, Wn=[80, 1600], btype="bandpass", fs=sample_rate, output="sos")
    ...
    def _calculate_statistics_with_outlier_removal(self, features: Dict[str, np.ndarray]) -> Dict[str, float]:
        """外れ値除去を含む統計計算"""
        statistics = {}
            ...
            if len(values) > 0:
                ...
                if len(valid_values) > 0:
                    ...
                    if len(filtered_values) > 0:
                        statistics[f'{feature_name}_mean'] = float(np.mean(filtered_values))
                        statistics[f'{feature_name}_std'] = float(np.std(filtered_values))
                        statistics[f'{feature_name}_p25'] = float(np.percentile(filtered_values, 25))
                        statistics[f'{feature_name}_p75'] = float(np.percentile(filtered_values, 75))
                    else:
                        # 外れ値除去後にデータがなくなった場合
...
class StatisticalCalibrator:
    ...
    def add_sample(self, sample: AudioSample):
        ...
        logger.info(f"{sample.label}サンプルを追加（特徴量数: {len(sample.statistics)}）")
    ...
    def calculate_optimal_thresholds(self) -> CalibrationResult:
        ...
        sample_counts = {label: sum(1 for s in self.samples if s.label == label) for label in labels}
        ...
        for feature in key_features:
            ...
            for label in labels:
                ...
                if samples:
                    ...
                    for stat in ['mean', 'std', 'p25', 'p75']:
                        if stat == 'std':
                            feature_analysis[feature][f'{label}_{stat}'] = float(np.std(values))
                        else:
                            percentile = {'mean': 50, 'p25': 25, 'p75': 75}[stat]
                            feature_analysis[feature][f'{label}_{stat}'] = float(np.percentile(values, percentile))
    ...
    def _calculate_thresholds(self, fa: Dict) -> RuleSettings:
        ...
        if 'rms' in fa and fa['rms']:
            noise_floor = fa['rms'].get('silence_mean', 0) + 2 * fa['rms'].get('silence_std', 0)
            breathing_level = fa['rms'].get('breathing_mean', 0.01)
            settings.energy_threshold = max(0.005, (noise_floor + breathing_level * 0.7) / 2)
    ...
    def _evaluate_separation_quality(self) -> Dict[str, float]:
        ...
        for feature in key_features:
            ...
            for opponent_label in ['breathing', 'conversation']:
                snore_values = [s.statistics.get(f'{feature}_mean', 0) for s in self.samples if s.label == 'snore']
                opponent_values = [s.statistics.get(f'{feature}_mean', 0) for s in self.samples if s.label == opponent_label]
    ...
    def _calculate_confidence_scores(self, separation_quality: Dict) -> Dict[str, float]:
        """信頼度スコア計算"""
        scores = {}
        scores['overall_separation'] = np.mean(list(separation_quality.values())) if separation_quality else 0.5
        
        sample_counts = {label: sum(1 for s in self.samples if s.label == label) for label in ['silence', 'breathing', 'snore', 'conversation']}
        scores['data_balance'] = 1.0 if all(count > 0 for count in sample_counts.values()) else 0.3
        
        snore_samples = [s for s in self.samples if s.label == "snore"]
        if snore_samples:
            avg_snore_energy = np.mean([s.statistics.get('rms_mean', 0) for s in snore_samples])
...
class AutoCalibrator:
    ...
    def __init__(self):
        ...
        self.stages = [
            ...
            ("軽いいびき音収集", 15.0, "軽いいびき音を意図的に作ってください。", "snore"),
            ("通常会話収集", 20.0, "マイクに向かって読み上げてください。", "conversation")
        ]
