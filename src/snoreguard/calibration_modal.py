...
class CalibrationModal:
    ...
    def __init__(self, parent, on_completion: Optional[Callable[[CalibrationResult], None]] = None):
        self.parent = parent
    ...
    def show(self):
        ...
        # ウィンドウを中央に配置
        self.modal_window.geometry("+%d+%d" % (
...
4. 通常会話 20秒"""
        ...
        # ステータスフレーム
        status_frame = ctk.CTkFrame(main_frame, fg_color=self.COLOR_CARD, corner_radius=8)
        ...
        # 進行状況
        progress_frame = ctk.CTkFrame(main_frame, fg_color=self.COLOR_CARD, corner_radius=8)
        ...
        # ステージ情報
        stage_frame = ctk.CTkFrame(main_frame, fg_color=self.COLOR_CARD, corner_radius=8)
        ...
        # 音量表示
        volume_frame = ctk.CTkFrame(main_frame, fg_color=self.COLOR_CARD, corner_radius=8)
        ...
        # 結果表示フレーム（初期状態は非表示）
        self.result_frame = ctk.CTkFrame(main_frame, fg_color=self.COLOR_CARD, corner_radius=8)
    ...
    def apply_and_close(self):
        """結果を適用して閉じる"""
        if hasattr(self, 'calibration_result') and self.calibration_result and self.on_completion:
    ...
    def _start_stage(self):
        """現在のステージを開始"""
        try:
            stage_name, duration, instructions, label = self.auto_calibrator.get_stage_info(self.current_stage)
            ...
            # 3秒後に録音開始
            self.modal_window.after(3000, lambda: self._start_recording(stage_name, duration))
    ...
    def _start_recording(self, stage_name: str, duration: float):
        ...
        def record_thread():
            success = self.auto_calibrator.recorder.record_stage_async(stage_name, duration)
            if not success:
                self.modal_window.after(0, lambda: self.status_var.set("録音に失敗しました"))
    ...
    def _update_progress(self, progress: float, remaining_time: float):
        ...
        self.status_var.set(f"録音中... 残り {remaining_time:.1f}秒 (全体: {stage_progress:.1%})")
    ...
    def _on_stage_completion(self, audio_data, rms_level: float, max_amplitude: float):
        """ステージ完了処理"""
        self.modal_window.after(0, self._process_completion, audio_data, rms_level, max_amplitude)
    ...
    def _process_completion(self, audio_data, rms_level: float, max_amplitude: float):
        ...
        try:
            ...
            if self.auto_calibrator.process_recorded_audio(audio_data, label):
                self.status_var.set(f"ステージ {self.current_stage + 1} 完了 (RMS: {rms_level:.4f})")
    ...
    def _complete_calibration(self):
        ...
        try:
            ...
            if self.calibration_result:
                confidence = self.calibration_result.confidence_scores.get('total_confidence', 0)
